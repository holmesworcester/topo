...
#   scripts/run_perf_serial.sh lowmem     # fast lowmem defaults
#   PERF_LOWMEM_POC_ENABLE=1 PERF_LOWMEM_RUN_LARGE_TARGET=1 PERF_LOWMEM_RUN_SMALL_BRACKET=1 PERF_LOWMEM_RUN_FILE_PROXY=1 scripts/run_perf_serial.sh lowmem
#
...
PERF_LOWMEM_BASELINE_SMALL="${PERF_LOWMEM_BASELINE_SMALL:-50000}"
...
PERF_LOWMEM_FILE_BASELINE="${PERF_LOWMEM_FILE_BASELINE:-50000}"
PERF_LOWMEM_FILE_COUNT="${PERF_LOWMEM_FILE_COUNT:-20}"
...
PERF_LOWMEM_POC_ENABLE="${PERF_LOWMEM_POC_ENABLE:-0}"
...
PERF_LOWMEM_RUN_SMALL_BRACKET="${PERF_LOWMEM_RUN_SMALL_BRACKET:-0}"
PERF_LOWMEM_RUN_LARGE_TARGET="${PERF_LOWMEM_RUN_LARGE_TARGET:-0}"
...
run_lowmem_matrix() {
  if [[ "${PERF_LOWMEM_RUN_SMALL_TARGET}" == "1" ]]; then
    run "Lowmem Delta (${PERF_LOWMEM_BASELINE_SMALL}+${PERF_LOWMEM_DELTA_TARGET} messages)" \
      env \
  ...
  if [[ "${PERF_LOWMEM_RUN_SMALL_BRACKET}" == "1" ]]; then
    run "Lowmem Delta (${PERF_LOWMEM_BASELINE_SMALL}+${PERF_LOWMEM_DELTA_BRACKET} messages)" \
      env \
      LOWMEM_PROXY_BASE_EVENTS="${PERF_LOWMEM_BASELINE_SMALL}" \
  ...
  if [[ "${PERF_LOWMEM_RUN_LARGE_TARGET}" == "1" ]]; then
    run "Lowmem Delta (${PERF_LOWMEM_BASELINE_LARGE}+${PERF_LOWMEM_DELTA_TARGET} messages)" \
      env \
  ...
  if [[ "${PERF_LOWMEM_RUN_FILE_PROXY}" == "1" ]]; then
    run "Lowmem Delta Files (${PERF_LOWMEM_FILE_BASELINE}+${PERF_LOWMEM_FILE_COUNT}x${PERF_LOWMEM_FILE_SIZE_MIB}MiB)" \
      env \
