...
Two scenarios govern a new joiner's access to historical messages:

**Case A — bundle retired causally before invite creation.** The inviter knows at invite-creation time which bundles are already retired on their local state (K_bundle plaintext gone from `key_secrets`, but K_m rows for the un-deleted messages in those bundles still present). The `key_history_bundle` event delivered with the invite carries a unified slot format: each slot is `(target_event_id, wrapped_32_bytes)` where `target_event_id` is either a KeyRotation/key_broadcast event id (for live bundles — slot unwraps to K_bundle) or a `message_key` event id (for un-deleted messages in retired bundles — slot unwraps to K_m). The projector unwraps each slot and writes the result into `key_secrets` keyed by `target_event_id`; the existing Kahn cascade dispatches correctly based on which events are blocked on that id. The joiner projects the history bundle first (it's delivered as part of the bootstrap flow), then syncs messages and decrypts normally. No separate event type, no per-case wire fork.

**Case B — bundle retired causally after invite creation, before joiner completes sync.** The invite-time `key_history_bundle` wraps the then-live K_bundle, but by the time the joiner projects it some retirement has happened on other peers and K_bundle has been shredded everywhere else. Naively unwrapping the slot would re-hydrate K_bundle plaintext in the joiner's `key_secrets`, undoing strong FS for that joiner — they become a decryption oracle for retained wire until their own `MessageDeletion` cascade arrives and purges the row. Handling for this case is open and tracked as an implementation-time question; the invariant "invite TTL < bundle retirement cadence" makes it rare but not impossible. See `PLAN.md` §22.3 for the current mitigation strategy.

**Case C — bundle is live throughout.** Common case. Invite wraps current K_bundle; joiner syncs, receives K_bundle via `key_history_bundle`, unwraps every `message_key` they then see, caches K_m per-message. No special handling needed. If a deletion arrives later, the joiner participates in the cascade like any other honest peer.
