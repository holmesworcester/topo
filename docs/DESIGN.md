...
9. forward-on-have live hint bus (`src/state/live_hints.rs`, enabled by `TOPO_FORWARD_ON_HAVE=1`) reduces fresh-event discovery latency from one negentropy round gap (default 5 s) to sub-10 ms by broadcasting `LiveHint` entries to active sessions immediately on canonical persist.
