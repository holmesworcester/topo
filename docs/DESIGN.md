...
A joiner who arrives after a bundle has been retired (because some message under it was deleted) cannot obtain the bundle — no honest peer has its `key_secrets` row any more. In the current design, **the un-deleted messages in a retired bundle are history-lost to late joiners.** Existing peers retain the plaintext of those messages in their `messages` / `Encrypted` projections, but the `message_key` wire events cannot be re-unwrapped by anyone who didn't materialize `K_m` before the retirement.

This is an accepted trade. A per-K_m targeted delivery event (analogous to `key_bundle_share` but wrapping `K_m` to a recipient's `WrapPubkey`) would close the gap, but it is deferred until a concrete product driver surfaces — many workspaces never have late joiners, or all deletions happen after all interested parties have already materialized the relevant K_m values. Bundles that never see a deletion (the common case) stay cheap to ship to any future joiner via bundle-level delivery.

The arithmetic still favors not rotating on delete even with history loss: rotating on every delete costs ~524 KB per emitted `key_broadcast` (8192 recipient slots) for the entire active population, whereas losing history for a retired bundle affects only the subset of joiners who show up after the deletion and who actually needed those specific undeleted messages. For typical chat workloads, "retire the bundle, late joiners miss that range" is the cheaper aggregate choice.
