...
Dependencies now ride the same range-owned sync model as hot objects. Each fixed UTC day shard maintains a durable claim set for out-of-range prerequisites relevant to roots in that shard; sessions reconcile claims first and then negotiate actual objects from the union of the selected base shards and locally present claimed objects. This keeps hot sync self-contained without a separate blocker-repair lane.

Legacy `forward_on_have` controls are no longer part of the active sync design. The tested path is claim sync followed by object sync; bulk transfer no longer uses durable `wanted` rows, `ResponseCredit`, a separate dependency-repair session, or a shared ingest channel to keep the wire busy.
