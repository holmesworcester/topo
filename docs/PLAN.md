...
5. **Model deterministic trust-anchor binding from explicit accepted-invite data**: `invite_accepted` binds trust anchor directly from its validated event fields (for example accepted `network_id`/workspace root id), not from pre-projection blob-capture side channels.
