...
\*   network_id field (first-write-wins; no pre-projection capture).
...
\*
\* Bootstrap sequence:
\*   1. invite_accepted (local, binds trust anchor from event's network_id)
...
\*   9. invite_accepted_bob (local, binds trust anchor from event's network_id)
...
\* In this concrete model, invite_accepted always references the "main" network.
