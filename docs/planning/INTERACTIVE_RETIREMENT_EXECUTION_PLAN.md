...
2. `accept-link`
3. `identity` (combined view for transport + active user/peer)
4. `workspaces` alias (keep `networks` as compatibility alias during cutover round, then remove old name in same round per POC replacement policy).
...
3. Channel aliases (`channels`, `new-channel`, `channel`) as daemon frontend state over existing channel IDs
4. DB selector aliases for all registered DBs: `--db <path|name|index>`
...
Out-of-scope note: interactive currently exposes `ban`, but we should remove it
rather than migrate it. Correct user removal requires key rotation and group key
agreement semantics that this PoC does not implement.
...
3. Message selector wrappers for `react`/`delete`.
4. Identity summary response.
