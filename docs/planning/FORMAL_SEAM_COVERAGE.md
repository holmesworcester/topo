...
| Encode/parse round-trip type-code | `src/event_modules/mod.rs::encode_event` (debug-assert after encode) | `verus-proofs/src/state/command_roundtrip.rs` (`event_type_code_preserved`) | every call to `encode_event` re-parses the produced blob in debug builds and asserts the type code matches the original; encoder/decoder drift fires in test runs | `AMF` | `cargo test -j1 --lib create:: -- --nocapture`; `cargo-verus verify` |
