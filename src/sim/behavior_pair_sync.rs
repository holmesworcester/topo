...
fn expand_transferable_shared_closure(
    ...
    while let Some(event) = pending.pop() {
        ...
        for (_field_name, dep_id) in parsed.dep_field_values() {
...
fn winning_key_shared_event_ids(
    ...
    for (event_id, event_type, _created_at_ms, blob) in behavior.shared_recorded_events()? {
        if event_type != "key_shared" {
            continue;
        }
...
fn observed_key_shared_targets(
    ...
    let mut out = BTreeSet::new();
    for (_event_id, event_type, _created_at_ms, blob) in behavior.shared_recorded_events()? {
        if event_type != "key_shared" {
            continue;
        }
        let Ok(ParsedEvent::KeyShared(event)) = events::parse_event(&blob) else {
