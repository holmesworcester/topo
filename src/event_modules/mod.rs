...
pub const EVENT_TYPE_ENDPOINT_SECRET: u8 = 30;
pub const EVENT_TYPE_ENDPOINT_SHARED: u8 = 31;
...
mod tests {
    ...
    fn test_registry_transport_privacy_coverage() {
        let reg = registry();
        let required_codes: Vec<u8> = (1..=31u8)
            .filter(|c| {
                reg.lookup(*c)
