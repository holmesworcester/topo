...
fn calculate_download_rate_mib_s(
    ...
    first_slice_recorded_at_ms: Option<i64>,
    last_slice_recorded_at_ms: Option<i64>,
) -> Option<f64> {
    if downloaded_bytes <= 0 {
        return None;
    }
    let start_ms = first_slice_recorded_at_ms?;
...
pub fn list_for_message(
    ...
    let mut stmt = db.prepare(
        "SELECT a.filename, a.mime_type, a.blob_bytes, a.total_slices,
                ...
                (SELECT MIN(re.recorded_at)
                 FROM file_slices fs
                 JOIN recorded_events re
                   ON re.peer_id = fs.recorded_by AND re.event_id = fs.event_id
                 WHERE fs.recorded_by = a.recorded_by AND fs.file_id = a.file_id) AS first_slice_recorded_at_ms,
    ...
    let rows = stmt
        .query_map(rusqlite::params![recorded_by, message_id_b64], |row| {
            ...
            let first_slice_recorded_at_ms: Option<i64> = row.get(6)?;
...
pub fn list_files(
    ...
    let files = stmt
        .query_map(rusqlite::params![recorded_by], |row| {
            ...
            let first_slice_recorded_at_ms: Option<i64> = row.get(10)?;
    ...
    fn test_slices_received_counts_correctly() {
        ...
        db.execute(
            "INSERT INTO recorded_events (peer_id, event_id, recorded_at, source)
             VALUES (?1, ?2, ?3, ?4)",
            rusqlite::params!["peer1", "slice_evt1", 1000i64, "quic_recv:alice"],
        )
        .unwrap();
        db.execute(
            "INSERT INTO recorded_events (peer_id, event_id, recorded_at, source)
             VALUES (?1, ?2, ?3, ?4)",
            rusqlite::params!["peer1", "slice_evt2", 2000i64, "quic_recv:alice"],
        )
        .unwrap();
        ...
        approx_eq(result[0].download_rate_mib_s.unwrap(), 0.5);
    }

    #[test]
    fn test_download_progress_uses_recorded_window_and_actual_slice_bytes() {
        let db = setup_db();
        db.execute(
            "INSERT INTO files
             (recorded_by, event_id, message_id, file_id, blob_bytes, total_slices, slice_bytes, root_hash, key_event_id, filename, mime_type, created_at)
        ...
        db.execute(
            "INSERT INTO file_slices (recorded_by, file_id, slice_number, event_id, created_at)
             VALUES (?1, ?2, ?3, ?4, ?5)",
            rusqlite::params!["peer1", "file1", 4i64, "slice_evt_last", 10000i64],
        )
        .unwrap();
        db.execute(
            "INSERT INTO file_slices (recorded_by, file_id, slice_number, event_id, created_at)
             VALUES (?1, ?2, ?3, ?4, ?5)",
            rusqlite::params!["peer1", "file1", 0i64, "slice_evt_first", 10001i64],
        )
        .unwrap();
        db.execute(
            "INSERT INTO recorded_events (peer_id, event_id, recorded_at, source)
             VALUES (?1, ?2, ?3, ?4)",
            rusqlite::params!["peer1", "slice_evt_last", 1000i64, "quic_recv:alice"],
        )
        .unwrap();
        db.execute(
            "INSERT INTO recorded_events (peer_id, event_id, recorded_at, source)
             VALUES (?1, ?2, ?3, ?4)",
            rusqlite::params!["peer1", "slice_evt_first", 3000i64, "quic_recv:alice"],
        )
        .unwrap();

        let by_message = list_for_message(&db, "peer1", "msg1").unwrap();
        assert_eq!(by_message.len(), 1);
        assert_eq!(by_message[0].slices_received, 2);
        assert_eq!(by_message[0].downloaded_bytes, 393216);
        approx_eq(by_message[0].download_rate_mib_s.unwrap(), 0.1875);

        let files = list_files(&db, "peer1", 10).unwrap();
        assert_eq!(files.total, 1);
        assert_eq!(files.files[0].slices_received, 2);
        assert_eq!(files.files[0].downloaded_bytes, 393216);
        approx_eq(files.files[0].download_rate_mib_s.unwrap(), 0.1875);
    }
