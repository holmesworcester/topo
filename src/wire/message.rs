...
/// - channel_id: [u8; 32]
/// - author_id: [u8; 32]
/// - prev_message_id: [u8; 32] (optional, zeros if none) - Blake2b-256 hash
...
impl MessagePayload {
    ...
    pub fn parse(input: &[u8]) -> IResult<&[u8], Self> {
        ...
        let mut prev_message_id = [0u8; 32];
        prev_message_id.copy_from_slice(prev_message_id_bytes);
    ...
    pub fn encode(&self, buf: &mut [u8]) {
        ...
        buf[64..96].copy_from_slice(&self.prev_message_id);
        buf[96..104].copy_from_slice(&self.created_at_ms.to_le_bytes());

        let content_bytes = self.content.as_bytes();
        let content_len = content_bytes.len().min(Self::CONTENT_MAX_LEN);
        buf[104..106].copy_from_slice(&(content_len as u16).to_le_bytes());
        buf[106..106 + content_len].copy_from_slice(&content_bytes[..content_len]);
        // reserved bytes 362..448 are already zero
    }

    /// Check if prev_message_id is set (non-zero)
    pub fn has_prev(&self) -> bool {
        self.prev_message_id != [0u8; 32]
    }
    ...
    pub fn new(
        ...
        Self {
            channel_id,
            author_id,
            prev_message_id: prev_message_id.unwrap_or([0u8; 32]),
            created_at_ms,
            content,
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_message_no_prev() {
        let msg = MessagePayload::new([1u8; 32], [2u8; 32], None, "test".to_string());
        assert!(!msg.has_prev());
    }

    #[test]
    fn test_message_with_prev() {
        let msg = MessagePayload::new([1u8; 32], [2u8; 32], Some([3u8; 32]), "test".to_string());
        assert!(msg.has_prev());
    }
