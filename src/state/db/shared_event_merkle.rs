...
fn build_merkle_nodes(keys: &[NodeKey]) -> (Option<NodeKey>, Vec<NodeRecord>) {
    if keys.is_empty() {
        return (None, Vec::new());
    }
    ...
    let mut nodes = keys
        .iter()
        .map(|key| NodeRecord::new(key.ts, key.id))
        .collect::<Vec<_>>();
    let mut stack: Vec<usize> = Vec::with_capacity(nodes.len());
    for index in 0..nodes.len() {
        let mut last = None;
        while let Some(&top) = stack.last() {
            if rank_cmp(keys[top], nodes[top].rank, keys[index], nodes[index].rank)
                == Ordering::Greater
            {
                break;
            }
    ...
    (Some(keys[root_index]), nodes)
}

impl SharedEventMerkleStorage {
    fn finalize_range(mut storage: Self, range: SyncWindow) -> Result<Self, String> {
    ...
    pub fn from_entries(entries: &[(i64, EventId)], range: SyncWindow) -> Result<Self, String> {
        let mut keys = entries
            .iter()
            .map(|(ts, id)| NodeKey { ts: *ts, id: *id })
            .collect::<Vec<_>>();
        keys.sort_unstable();
        keys.dedup();
        let (root, nodes) = build_merkle_nodes(&keys);
        Self::finalize_range(
            Self {
                root,
                global_begin: 0,
...
pub fn delete_event_id(conn: &Connection, event_id: &EventId) -> SqliteResult<()> {
    ...
    if rows.is_empty() {
        ...
        conn.execute(
            "DELETE FROM shared_priority_event_index WHERE id = ?1",
            params![event_id.as_slice()],
        )?;
        conn.execute(
...
fn rebuild_workspace_index(conn: &Connection, workspace_id: &str) -> SqliteResult<()> {
    ...
    let (root_key, nodes) = build_merkle_nodes(&keys);
