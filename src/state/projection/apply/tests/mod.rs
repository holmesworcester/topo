...
fn test_encrypted_parity_file_slice_valid() {
    ...
    // Create descriptor (required for file_slice projection) — returns key info
    let file_id = [99u8; 32];
    let (_att_eid, desc_sk_eid, desc_key_bytes) =
        setup_descriptor_for_file(&conn, recorded_by, &signing_key, &signer_eid, file_id);

    // Create and encrypt file_slice using descriptor's key
    ...
    let (_enc, enc_blob) =
        make_encrypted_event(&desc_key_bytes, &fs_blob, EVENT_TYPE_FILE_SLICE, &desc_sk_eid);
    let enc_eid = insert_event_raw(&conn, recorded_by, &enc_blob);
    let result = project_one(&conn, recorded_by, &enc_eid).unwrap();
    assert_eq!(result, ProjectionDecision::Valid);
...
/// Returns (attachment_event_id, key_secret_event_id, key_bytes).
fn setup_descriptor_for_file(
    ...
    signer_eid: &EventId,
    file_id: [u8; 32],
) -> (EventId, EventId, [u8; 32]) {
    let key_bytes = [0xBB; 32];
    let sk = ParsedEvent::KeySecret(KeySecretEvent {
        created_at_ms: now_ms(),
        key_bytes,
    });
    ...
    let sk_eid = insert_event_raw(conn, recorded_by, &sk_blob);
    project_one(conn, recorded_by, &sk_eid).unwrap();
    setup_descriptor_for_file_with_key(conn, recorded_by, signing_key, signer_eid, file_id, sk_eid, key_bytes)
}

/// Like setup_descriptor_for_file but uses a pre-existing key_secret.
fn setup_descriptor_for_file_with_key(
    ...
    (att_eid, sk_eid, key_bytes)
}

/// Helper: wrap a signed file_slice blob in an encrypted envelope and insert it.
fn insert_encrypted_file_slice(
    conn: &Connection,
    recorded_by: &str,
    fs_blob: &[u8],
    key_bytes: &[u8; 32],
    ...
    let (_enc, enc_blob) = make_encrypted_event(key_bytes, fs_blob, EVENT_TYPE_FILE_SLICE, sk_eid);
...
fn test_file_slice_valid() {
    ...
    let file_id = [99u8; 32];
    let (_att_eid, sk_eid, key_bytes) =
        setup_descriptor_for_file(&conn, recorded_by, &signing_key, &signer_eid, file_id);

    // Create FileSlice (encrypted)
    let (_fs, fs_blob) = make_file_slice(&signing_key, &signer_eid, file_id, 0, b"encrypted data");
    let fs_eid = insert_encrypted_file_slice(&conn, recorded_by, &fs_blob, &key_bytes, &sk_eid);
...
#[test]
fn test_plaintext_file_slice_rejected_by_require_encrypted() {
    // A file_slice sent as bare plaintext (not wrapped in encrypted) must be rejected.
    let conn = setup();
    let recorded_by = "peer1";
    let (signer_eid, signing_key) = make_identity_chain(&conn, recorded_by);
    let file_id = [99u8; 32];
    ...
    let (_fs, fs_blob) = make_file_slice(&signing_key, &signer_eid, file_id, 0, b"plaintext data");
    let fs_eid = insert_event_raw(&conn, recorded_by, &fs_blob);
    let result = project_one(&conn, recorded_by, &fs_eid).unwrap();
    assert!(
        matches!(result, ProjectionDecision::Reject { ref reason } if reason.contains("requires encrypted transport")),
        "plaintext file_slice should be rejected, got {:?}",
        result
    );
}
...
fn test_file_slice_blocks_on_missing_signer() {
    ...
    // Create a key_secret for encryption
    let key_bytes: [u8; 32] = rand::random();
    let (_sk, sk_blob) = make_key_secret(key_bytes);
    let sk_eid = insert_event_raw(&conn, recorded_by, &sk_blob);
    project_one(&conn, recorded_by, &sk_eid).unwrap();
...
fn test_file_slice_unblocks_when_signer_arrives() {
    ...
    // Create a key_secret for encryption (must exist before encrypted file_slice)
    let key_bytes: [u8; 32] = rand::random();
    let (_sk, sk_blob) = make_key_secret(key_bytes);
    let sk_eid = insert_event_raw(&conn, recorded_by, &sk_blob);
    project_one(&conn, recorded_by, &sk_eid).unwrap();

    // Create FileSlice referencing the not-yet-existing signer (encrypted)
    let file_id = [99u8; 32];
    ...
    // Now create the descriptor using the SAME key_secret — this should cascade-unblock the file_slice
    setup_descriptor_for_file_with_key(&conn, recorded_by, &signing_key, &signer_eid, file_id, sk_eid, key_bytes);
...
fn test_multiple_slices_same_file() {
    ...
    for i in 0..5u32 {
        ...
        let fs_eid =
            insert_encrypted_file_slice(&conn, recorded_by, &fs_blob, &key_bytes, &sk_eid);
        let result = project_one(&conn, recorded_by, &fs_eid).unwrap();
        assert_eq!(
            result,
            ProjectionDecision::Valid,
...
fn test_file_slice_tenant_isolation() {
    ...
    let fs_eid =
        insert_encrypted_file_slice(&conn, "tenant_a", &fs_blob, &key_bytes, &sk_eid);
...
fn test_file_slice_duplicate_slot_conflict_rejects() {
    ...
    // First slice at slot 0 (encrypted)
    let (_fs1, fs1_blob) = make_file_slice(&signing_key, &signer_eid, file_id, 0, b"first");
    let fs1_eid =
        insert_encrypted_file_slice(&conn, recorded_by, &fs1_blob, &key_bytes, &sk_eid);
    let result = project_one(&conn, recorded_by, &fs1_eid).unwrap();
    assert_eq!(result, ProjectionDecision::Valid);

    // Second, DIFFERENT slice at same slot 0 — should reject (encrypted)
    let (_fs2, fs2_blob) = make_file_slice(&signing_key, &signer_eid, file_id, 0, b"second");
    let fs2_eid =
        insert_encrypted_file_slice(&conn, recorded_by, &fs2_blob, &key_bytes, &sk_eid);
...
fn test_file_slice_wrong_signer_rejected() {
    ...
    // Create file_slice signed by signer B (different from descriptor's signer A), encrypted
    let (_fs, fs_blob) = make_file_slice(
        &signer_key_b,
        &signer_b_eid,
        file_id,
...
fn test_file_slice_guard_retry_after_cascaded_attachment() {
    ...
    // Create file_slice (encrypted) — guard-blocked (no descriptor yet)
