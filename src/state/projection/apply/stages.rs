...
use crate::projection::queries::{ContextLoadResult, DepLoadResult};
...
fn load_context_with_prereqs<B: ProjectionBackend>(
    ...
    (meta.context_loader)(backend, recorded_by, event_id_b64, parsed)
}
...
fn apply_projection_frame<B: ProjectionBackend>(
    ...
    let mut ctx = match load_context_with_prereqs(backend, recorded_by, event_id_b64, parsed)? {
    ...
    // Signer verification stays generic, but it now runs after the tri-state
    // prerequisite load so missing signer deps can still block instead of
    // degrading into signer-not-found rejection.
