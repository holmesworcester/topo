...
/// Legacy static coordinator entrypoint retained for existing test/util paths.
///
/// New runtime code uses [`CoordinationManager`], but `download_from_sources`
/// still relies on this helper in some test harnesses.
    ...
    loop {
        ...
        while reported_count < total_peers && Instant::now() < deadline {
            for (i, rx) in report_rxs.iter().enumerate() {
                ...
                if let Ok(need_ids) = rx.try_recv() {
                    reports[i] = Some(need_ids);
                    reported_count += 1;
                }
            }
