...
fn summarize_sync_event_detail(frame_type: &str, detail_json: Option<&str>) -> String {
    ...
    match frame_type {
        ...
        "ExactDiffHints" => {
            let count = v["hint_count"].as_u64().unwrap_or(0);
            let bound_ts = v["bound_ts"].as_u64().unwrap_or(0);
            let bound_prefix = v["bound_id_prefix"]
                .as_str()
                .map(short_sync_id)
                .unwrap_or_default();
            let ids = v["ids"]
            ...
            let bound = if bound_prefix.is_empty() {
                format!("bound=({bound_ts})")
            } else {
                format!("bound=({bound_ts}, {bound_prefix})")
            };
            if ids.is_empty() {
                format!(
                    " detail=exact_diff({} count={} truncated={})",
                    bound, count, truncated
                )
            } else {
                let shown = ids.join(",");
                let shown_count = ids.len() as u64;
                ...
                format!(
                    " detail=exact_diff({} count={} truncated={} ids=[{}]{})",
                    bound, count, truncated, shown, extra
                )
            }
        }
        "ResponseCredit" => {
