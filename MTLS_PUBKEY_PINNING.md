...
This document describes mutual TLS where acceptable peer identities are controlled by pinned public keys. This is implemented in the QUIC transport via a pluggable keystore interface.
...
- Support large peer sets without loading all keys into memory.
...
**Observability**
On startup, the server/client logs its own SPKI (base64). This lets you copy/paste the peer SPKI into `--peer-spki` for pinning.
