...
//! with mDNS) to maximise the probability that at least one hits the race.
//! Without the fix the race triggers ~50 % per-peer (depends on peer-id
//! ordering), so with 4 invitees the chance of at least one failure is ~94 %.
...
fn invite_bootstrap_completes_with_mdns_active() {
    ...
    for (i, peer) in peers.iter().enumerate() {
        ...
        // Also verify identity materialized (transport converged to PeerShared)
        assert_identity_eventually_materialized(&peer.db, timeout_ms);

        // Verify the invitee can send a message back to the inviter
