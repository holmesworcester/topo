...
use topo::peering::workflows::intro::{run_intro, send_intro_offer, build_intro_offer};
...
/// across peers through normal sync.
...
///    and relays identity chains so PeerShared trust projects at each peer)
...
async fn test_three_peer_intro_happy_path() {
    ...
    // project trusted PeerShared identities at each peer after sync.
    ...
    // Wait for peer identity trust projection to complete at A and B.
    {
        ...
        assert_eventually(
            ...
            "peer identity trust projection at A and B",
        ).await;
    }
