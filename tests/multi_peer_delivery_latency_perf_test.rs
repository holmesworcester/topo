...
fn lookup_recorded_at(peer: &Peer, event_id_b64: &str) -> i64 {
    let conn = topo::db::open_connection(&peer.db_path).expect("open peer db for recorded_at");
    conn.query_row(
        "SELECT recorded_at
...
fn collect_delivery_samples(peers: &[Peer], sent_messages: &[SentMessage]) -> Vec<DeliverySample> {
    ...
    for sent in sent_messages {
        ...
        let recorded_at = lookup_recorded_at(sink, &sent.event_id_b64);
        let latency_ms = recorded_at - sent.created_at_ms;
        assert!(
            latency_ms >= 0,
            "negative delivery latency for event {} origin={} sink={} created_at_ms={} recorded_at={}",
            sent.event_id_hex,
            origin.name,
            sink.name,
            sent.created_at_ms,
...
// ---------------------------------------------------------------------------
// Discovery-mode comparison: current round-based discovery
// ---------------------------------------------------------------------------

/// Negentropy only: default 5 s rounds.  Shows the current production latency.
#[tokio::test]
#[ignore]
