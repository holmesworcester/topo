    ...
    T: NegentropyStorageBase,
{
    ...
    ///
    /// Like `reconcile()` but also populates `have_ids` (IDs the server has
    /// that the client appears to lack) and `need_ids` (IDs the client has
    /// that the server appears to lack). This lets the server send metadata
    /// hints for the diff without a separate round trip.
